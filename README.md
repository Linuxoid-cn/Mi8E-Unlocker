...
* **机型目录对齐：** 脚本运行后会根据你选择的机型自动调用对应文件夹（如 `Redmik90`、`Xiaomi15ultra` 等）内的特定配置文件。
...
2.  **启动脚本：** 在 Windows 电脑解压本工具包，双击运行目录下的 `toUnlock.bat'
...
如果你在上述 6 款机型上遇到“未识别设备”或脚本报错，请提交 **Issue** 并附上报错截图，我会针对该机型文件夹内的逻辑进行修复。
## 👥 贡献者名单 (Contributors)
